...
namespace Mediapipe.Unity.Tutorial
{
  public class PoseTest : MonoBehaviour
  {
    ...
    private IEnumerator Start()
    {
      ...
      sidePacket.Emplace("input_rotation", new IntPacket((int)0));
      sidePacket.Emplace("input_horizontally_flipped", new BoolPacket(true));
      sidePacket.Emplace("input_vertically_flipped", new BoolPacket(true));
      Debug.Log(sidePacket);

      _graph = new CalculatorGraph(_configAsset.text);
      var outputVideoStream = new OutputStream<ImageFramePacket, ImageFrame>(_graph, "throttled_input_video");
      ...
      while (true)
      {
        ...
        if (multiFaceLandmarksStream.TryGetNext(out var multiFaceLandmarks))
        {
          if (multiFaceLandmarks != null && multiFaceLandmarks.Count > 0)
          {
            ...
            {
              // top of the head
              var topOfHead = landmarks.Landmark[11];
